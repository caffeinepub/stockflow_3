...
function DeliveryTab({
  ...
  const handleSaveDelivery = () => {
    ...
    // Deduct stock from godown for each item
    for (const item of validItems) {
      const existingItem = Object.values(inventory).find(
        (inv) =>
          (!inv.businessId || inv.businessId === activeBusinessId) &&
      ...
      if (existingItem) {
        setInventory((prev) => {
          const current = prev[existingItem.sku];
          if (!current) return prev;
          const godownQty = current.godowns[selectedGodown] || 0;
          const deduct = Math.min(Number(item.qty), godownQty);
          return {
            ...prev,
            [existingItem.sku]: {
              ...current,
              godowns: {
                ...current.godowns,
                [selectedGodown]: godownQty - deduct,
              },
            },
          };
        });
